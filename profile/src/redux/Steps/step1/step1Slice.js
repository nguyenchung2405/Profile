  ...
  reducers: {
    addPBCV: (state, action) => {
      ...
      if (payload.length > 0) {
        ...
        let index = state.phongBanChucVuArr.findIndex(
          (PB_CV) =>
            PB_CV.phongBan === payload.phongBan &&
            PB_CV.chucVu === payload.chucVu&&
            PB_CV.to === payload.to
        );
        if (index === -1) {
    ...
    setNoiSinhHuyen: (state, action) => {
      let quanSelected = state.noiSinhQuan.find(
        (quan) => quan.name === action.payload
      );
    ...
    setQueQuanHuyen: (state, action) => {
      let quanSelected = state.queQuanQuan.find(
        (quan) => quan.name === action.payload
      );
    ...
    setNoiOHuyen: (state, action) => {
      let quanSelected = state.noiOQuan.find(
        (quan) => quan.name === action.payload
      );
    ...
    setHoKhauHuyen: (state, action) => {
      ...
      let quanSelected = state.hoKhauQuan.find(
        (quan) => quan.name === action.payload
      );
