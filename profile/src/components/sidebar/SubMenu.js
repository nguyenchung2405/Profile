...
export default function SubMenu(permission) {
  ...
  const PermissionPosition = () => (
    ...
    >
      <g>
        ...
        <path d="M727,464.2c-145.3,0-263,117.7-263,263s117.7,263,263,263c145.3,0,263-117.7,263-263C990,581.9,872.3,464.2,727,464.2z M879,835.1c0,39.2-75.2,48.2-145.8,48.2c-70.6,0-146.8-9-146.8-48.2v-10.9c0-28.9,52.1-58.2,96.3-74.1c3.3-1.1,18.6-9.6,10-32c-22-22.4-38.5-58.8-38.5-94.6c0-54.7,36.2-83.5,78.7-83.5c42.3,0,78.9,28.7,78.9,83.5c0,35.6-16.7,71.8-38.5,94.4h0.2c-8.4,25.5,5,31.6,7.1,32.2c46.5,15.3,98.8,44.4,98.8,74.1L879,835.1L879,835.1z" />
      </g>
    </svg>
  );
  ...
  const getUserId = () => {
    if (
      docCookies.getItem("emails") &&
      docCookies.getItem("emails") === "erpmanager@tuoitre.com.vn"
    ) {
      return true;
    } else {
      return false;
  ...
  // getItem(<Link to={`${uri}/hr/profile/${Math.random().toString(36).substring(7)}`}>Hồ sơ</Link>, "100", <BsFileEarmarkFill />),
    ...
    const items = [
      getItem("Nhân sự", "sub-menu-1", <MdPeopleOutline />, [
        getItem(
          <Link to={`${uri}/hr/profile`}>Hồ sơ</Link>,
          "100",
          <BsFileEarmarkFill />
        ),
        getItem(
          <Link to={`${uri}/hr/department`}>Bộ phận công tác</Link>,
          "102",
          <FcDepartment />
        ),
        getItem(
          <Link to={`${uri}/hr/position`}>Chức danh, chức vụ</Link>,
          "103",
          <PositionItem />
        ),
        getItem(
      ...
      getItem("Quyền", "sub-menu-2", <PermissionMenu />, [
        getItem(
          <Link to={`${uri}/hr/permission/manage`}>Quản lý quyền</Link>,
          "105",
          <PermissionManagement />
        ),
        getItem(
          <Link to={`${uri}/hr/permission/position`}>Quyền chức vụ</Link>,
          "106",
          <PermissionPosition />
        ),
        getItem(
          <Link to={`${uri}/hr/permission/department-position`}>
            Quyền PB - CV
          </Link>,
          "107",
          <PermissionDepPos />
